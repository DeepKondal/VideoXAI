...
def process_video(video_path, output_dir, extractor, sampling_rate=2, temporal_smoothing_window=5):
    ...
    # Extract exactly 72 frames
    for frame in container.decode(video=0):
        frames.append(frame.to_rgb().to_ndarray())

    num_extracted_frames = 72
    total_frames = len(frames)
    if total_frames > num_extracted_frames:
        indices = np.linspace(0, total_frames - 1, num_extracted_frames, dtype=int)
        frames = [frames[i] for i in indices]
    
    for i in range(0, len(frames), 8):
        batch = frames[i:i + 8]
        if len(batch) < 8:
            batch.extend([batch[-1]] * (8 - len(batch)))  # Pad if necessary

        spatial_attention = multi_scale_attention(extractor, batch)
        logits = extractor.extract_attention(batch)[1]
        all_logits.append(logits)
        
        for j in range(8):
            attention = spatial_attention[0, j+1]
            ...
            if len(attention_buffer) >= temporal_smoothing_window:
                ...
                heatmap_frame = extractor.apply_attention_heatmap(batch[j], smoothed_attention)
    ...
    # Compute overall logits and predicted label
    overall_logits = np.mean(all_logits, axis=0) if all_logits else np.zeros(1)
...
def create_sample_frames_visualization(video_name, num_segments=8, results_dir='attention_results'):
    try:
        ...
        max_window_length = min(len(temporal_attention), 21)  # Limit max window size to 21 but not exceed available frames
        window_length = max(3, max_window_length // 2 * 2 + 1)  # Ensure it's odd and at least 3

        # Apply filter only if sufficient data points exist
        if len(temporal_attention) >= window_length:
            temporal_attention_smoothed = savgol_filter(temporal_attention, window_length, 3)
        else:
            temporal_attention_smoothed = temporal_attention 
