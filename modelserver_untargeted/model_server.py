...
os.makedirs(os.path.join(OUTPUT_DIR, "untargeted"), exist_ok=True)



@app.post("/facebook/timesformer-base-finetuned-k400/process-untargeted-videos")
async def process_timesformer_original_videos():
    ...
    if not os.path.exists(UNTARGETED_DIR):
        raise HTTPException(status_code=404, detail="Original video directory not found.")

    video_files = [f for f in os.listdir(UNTARGETED_DIR) if f.endswith(".mp4")]
    
    if not video_files:
        return {"message": "No original videos found in directory."}

    for video_file in video_files:
        video_path = os.path.join(UNTARGETED_DIR, video_file)
        ...
        try:
            ...
            # Save results
            video_result_dir = os.path.join(OUTPUT_DIR, "untargeted", os.path.splitext(video_file)[0])
            ...
            timesformer_extractor.visualize_attention(
                spatial_attention, temporal_attention, frames, video_result_dir, prediction, "untargeted"
            )
            
            results.append({
                "video_file": video_file,
                "video_type": "untargeted",
...
@app.post("/process-untargeted-videos")
async def process_and_visualize_original_videos():
    """
    This endpoint processes only targeted  videos using Temporal-Spatial XAI.
    """
    try:
        response = {"message": "Processing untargeted videos", "untargeted": []}

        # Process Clean Videos
        if os.path.exists(UNTARGETED_DIR):
            clean_videos = [f for f in os.listdir(UNTARGETED_DIR) if f.endswith(".mp4")]
            for video_name in clean_videos:
                clean_video_path = os.path.join(UNTARGETED_DIR, video_name)
                clean_output_dir = os.path.join(OUTPUT_DIR, "untargeted", video_name)
